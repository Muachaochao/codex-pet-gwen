...
一个可安装到 Codex 桌面应用的高清 3D 格温风格甜妹宠物包。动作增强版将桌面宠物的六个核心动作完整映射到 Codex 的九行固定动画槽，并保留成品宠物、动作预览、主要参考图和生成提示词，方便换电脑后直接恢复或继续迭代。
...
| 桌面宠物动作 | Codex 固定状态 | 动作增强版表现 |
| --- | --- | --- |
| 待机 | `idle` | 呼吸、眨眼和轻微重心变化 |
| 散步 | `running-right` / `running-left` | 左右方向的完整移动循环 |
| 敲代码 | `running` | 手持银灰色便携键盘持续工作 |
| 探头观察 | `review` | 左右探头、眯眼和歪头检查 |
| 庆祝成功 | `jumping` | 蹲下、腾空双手庆祝和落地 |
| 发现错误 | `failed` | 受惊、低头、抱膝沮丧和恢复 |

Codex 版还额外保留 `waving`（问候）和 `waiting`（等待用户输入）两个状态。Codex 宠物格式的状态行由应用固定，不能通过增加第十行来创建新的触发状态，因此增强版把桌面动作放进最匹配的官方状态槽位。
